...
void RealizarPedido(char productos[5][30], float cantidadesPorProducto[5][10], char ingredientes[10][30], float stock[10],
    ...
    if (confirmar == 's' || confirmar == 'S') {
        printf("\n--- Tabla de Ingredientes Descontados ---\n");
        ...
        printf("%-20s | %-20s | %-25s\n", "Producto", "Ingrediente", "Tiempo Producción (min)");
        printf("----------------------|----------------------|-------------------------\n");
        for (int j = 0; j < numIngredientes; j++) {
            if (cantidadesPorProducto[productoIndex][j] > 0) {
                printf("%-20s | %-20s | %-25.2f\n", nombreProducto, ingredientes[j], tiemposProduccion[productoIndex]);
            }
        }
...
void RealizarPedido(char productos[5][30], float cantidadesPorProducto[5][10], char ingredientes[10][30], float stock[10],
    ...
    // Tabla 2: Productos, ingredientes y tiempo de producción
    ...
    for (int i = 0; i < numProductos; i++) {
        for (int j = 0; j < numIngredientes; j++) {
            if (cantidadesPorProducto[i][j] > 0) {
                printf("%-20s | %-20s | %-25.2f\n", productos[i], ingredientes[j], tiemposProduccion[i]);
            }
        }
    }
